...
w_ref = w_nom_ref*((t-3).*heaviside(t-3)-(t-4).*heaviside(t-4)) - w_nom_ref*((t-4).*heaviside(t-4)-(t-5).*heaviside(t-5)) ...
    - w_nom_ref*((t-5.3).*heaviside(t-5.3)-(t-5.5).*heaviside(t-5.5));
...
modelFolder = './saved_model/speed_estimation/my_model_2' ;

scalerData = load('./saved_model/data/speed_estimation/my_model_2/scaler_values.mat');
...
for k = 1:Np
    ...
    input_data = [Iqs, Ids, Vq, Vd]; % e outros valores de entrada
    ...
    prediction_normalized = predict(net, normalized_data);

    % Desnormalizar os dados de saída
    wr_prediction = denormalizeOutput(prediction_normalized, scalerData.y_min, scalerData.y_max);

    %Velocidade
    e_w = w_ref(k) - wr;
    % e_w = w_ref(k) - wr_prediction;
    UI_w = UI_w + e_w*Tsc;
    ...
    Vq_vetor(k) = Vq;
    wr_prediction_vetor(k) = wr_prediction;
...
% Gráfico 2: wr_vetor e w_ref
figure;
plot(t, wr_vetor, t, wr_prediction_vetor, 'LineWidth', lineWidth);
title('Wr vs Ref', 'FontSize', fontSize);
xlabel('Tempo (s)', 'FontSize', fontAxis);
ylabel('Valores', 'FontSize', fontAxis);
legend('Wr', 'Prediction');
